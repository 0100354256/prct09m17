...
describe Preguntas do
 before :each do
   ...
   @p1 = SeleccionSimple.new(0, "Pregunta 1", respuestas)
 end
...
describe List do
 before :each do
  ...
  @p1 = SeleccionSimple.new(1, "Cual es la salida del siguiente codigo Ruby? \n class Xyz \n def pots \n @nice \n end \n end \n xyz = Xyz.new \n p xyz.pots", respuestas1)
  @p2 = VerdaderoYFalso.new(1, "La siguiente definicion de un hash en Ruby es valida: \n hash_raro = { \n [1, 2, 3] => Object.new(), \n Hash.new => :toto \n }")
  @p3 = SeleccionSimple.new(2, "Cual es la salida del siguiente codigo Ruby?", respuestas3)
  @p4 = SeleccionSimple.new(2, "Cual es el tipo del objeto en el siguiente codigo Ruby? \n class Objeto \n end", respuestas4)
  @p5 = VerdaderoYFalso.new(3, "Es apropiado que una clase Tablero herede de una clase Juego")
 ...
 describe "Lista de Preguntas" do
  ...
  it "Pregunta 1 - Con Enumerable" do
    expect(@n4.first.to_s).to eq("Cual es la salida del siguiente codigo Ruby? \n class Xyz \n def pots \n @nice \n end \n end \n xyz = Xyz.new \n p xyz.pots \n #<Xyz:0xa000208>\nnil\n0\nNinguna de las anteriores \n")
  end
  it "Maximo - Con Enumerable" do
    expect(@n4.max).to eq(@p5)
  end
 end
 ...
 describe "Comparando Preguntas" do
  ...
  it "# Menor o igual grado de dificultad" do
    expect(@p1 <= @p3).to be(true)
  end
  it "# Mayor o igual grado de dificultad" do
    expect(@p5 >= @p4).to be(true)
  end
  it "# Es Comparable?" do
   expect(@p1).to be_a Comparable
  end
  it "# Es Enumerable?" do
   expect(@n4).to be_a Enumerable
  end
 end
end